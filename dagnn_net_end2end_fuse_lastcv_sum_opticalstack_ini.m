function nn = dagnn_net_end2end_fuse_lastcv_sum_opticalstack_ini(varargin)
% Author: Jiawei Chen
% Two stream ConvNet fused after last Conv layer using sum fusion 
...
%------------Temporal Block3--------------------------------
t_cnv3 = dagnn.Conv('size',[5 5 64 128],'pad',2,'stride',1,'hasBias',true);
nn.addLayer('t_cnv3', t_cnv3, {'dp4'}, {'xc9_2'},{'c7','b7'}); % output dim = 30-6 = 24
nn.addLayer('t_bn3', dagnn.BatchNorm('numChannels',128), {'xc9_2'}, {'xc10_2'},{'bn6f', 'bn6b', 'bn6m'});
...
%------------Fusion Block----------------------------------------
%--do sum on the first fully connected layers
nn.addLayer('Summation', dagnn.Sum(), {'xc12_1', 'xc12_2'}, {'cv_sum'}); 
% nn.addLayer('Sum', dagnn.Sum(), {'xc14_1', 'xc14_2'}, {'cv_concat'}); 
%------------ fully connect 1-------------------------------------
f_fc1 = dagnn.Conv('size',[4 4 128 128],'pad',0,'stride',1,'hasBias',true);
nn.addLayer('f_fc1', f_fc1, {'cv_sum'}, {'fc1'}, {'sum','fc1'});
nn.addLayer('dropout5', dagnn.DropOut('rate', 0.5), {'fc1'}, {'dp5'}); 
%------------ fully connect 2--------------------------------------
f_fc2 = dagnn.Conv('size',[1 1 128 12],'pad',0,'stride',1,'hasBias',true);
nn.addLayer('f_fc2', f_fc2, {'dp5'}, {'pred'}, {'dp5_input','prediction'});
