function nn = dagnn_net_end2end_fuse_lastcv_concate_ini(varargin)
% Author: Jiawei Chen
% Two stream ConvNet fused after last Conv layer using concatenation fusion 
...
%------------Fusion Block----------------------------------------
nn.addLayer('concate', dagnn.Concat('dim',3), {'xc12_1', 'xc12_2'}, {'cv_concat'}); 
% f_fcc = dagnn.Conv('size', [1,1,256,128],'pad',0,'stride',1,'hasBias',true);
% nn.addLayer('f_fcc',f_fcc, {'cv_concat'}, {'conv_fc1'},{'s','t'}); 
nn.addLayer('f_re1', dagnn.ReLU(), {'cv_concat'}, {'fc1_relu'});
% nn.addLayer('dropout5', dagnn.DropOut('rate', 0.5), {'fc1_relu'}, {'dp5'}); 

%------------ fully connect 1-------------------------------------
f_fc1 = dagnn.Conv('size',[4 4 256 128],'pad',0,'stride',1,'hasBias',true);
nn.addLayer('f_fc1', f_fc1, {'fc1_relu'}, {'fc1'}, {'sum','fc1'});
